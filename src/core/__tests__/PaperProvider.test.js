...
import { MD2LightTheme, MD2DarkTheme, MD3LightTheme, MD3DarkTheme, } from '../../styles/themes';
import PaperProvider from '../PaperProvider';
import { useTheme } from '../theming';
jest.mock('react-native-safe-area-context', () => mockSafeAreaContext);
const mockAppearance = () => {
    jest.mock('react-native/Libraries/Utilities/Appearance', () => {
        const realApp = jest.requireActual('react-native/Libraries/Utilities/Appearance');
        const listeners = [];
        return {
            ...realApp,
...
const mockAccessibilityInfo = () => {
    jest.mock('react-native/Libraries/Components/AccessibilityInfo/AccessibilityInfo', () => {
        const realApp = jest.requireActual('react-native/Libraries/Components/AccessibilityInfo/AccessibilityInfo');
        const listeners = [];
        return {
            __esModule: true,
            default: {
...
const FakeChild = () => {
    ...
    return React.createElement(View, { testID: "provider-child-view", theme: theme });
};
const createProvider = (theme) => {
    return (React.createElement(PaperProvider, { theme: theme },
        React.createElement(FakeChild, null)));
};
...
describe('PaperProvider', () => {
    ...
    it('handles theme change', async () => {
        ...
        expect(getByTestId('provider-child-view').props.theme).toStrictEqual(ExtendedLightTheme);
        act(() => Appearance.__internalListeners[0]({ colorScheme: 'dark' }));
        expect(getByTestId('provider-child-view').props.theme).toStrictEqual(ExtendedDarkTheme);
    });
    ...
    it('should set AccessibilityInfo listeners, if there is no theme', async () => {
        ...
        act(() => AccessibilityInfo.__internalListeners[0]({
            reduceMotionEnabled: true,
        }));
        expect(getByTestId('provider-child-view').props.theme.animation.scale).toStrictEqual(0);
  ...
  `('provides $label for $colorScheme color scheme', async ({ theme, colorScheme }) => {
        mockAppearance();
        Appearance.getColorScheme.mockReturnValue(colorScheme);
        const { getByTestId } = render(createProvider());
        expect(getByTestId('provider-child-view').props.theme).toStrictEqual(theme);
    });
    it('uses provided custom theme', async () => {
        ...
        expect(getByTestId('provider-child-view').props.theme).toStrictEqual(customTheme);
    });
    it.each `
    ...
    ${3}    | ${'dark'}   | ${MD3DarkTheme}
  `('uses correct theme, $colorScheme mode, version $version', async ({ version, colorScheme, expectedTheme }) => {
        mockAppearance();
        Appearance.getColorScheme.mockReturnValue(colorScheme);
        const { getByTestId } = render(createProvider({ version }));
        expect(getByTestId('provider-child-view').props.theme).toStrictEqual(expectedTheme);
    });
});
