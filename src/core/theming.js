...
export const { ThemeProvider, withTheme, useTheme: useAppTheme, } = createTheming(MD3LightTheme);
...
export function adaptNavigationTheme(themes) {
    const { reactNavigationLight, reactNavigationDark, materialLight, materialDark, } = themes;
    ...
    if (reactNavigationLight && reactNavigationDark) {
        ...
        const { light: adaptedLight, dark: adaptedDark } = modes.reduce((prev, curr) => {
            return {
                ...prev,
        ...
        return {
            ...
            [`level${index}`]: index === 0
                ...
                : color(scheme.surface)
                    .mix(color(scheme.primary), elevationValue)
                    .rgb()
                    .string(),
        };
    }, {});
