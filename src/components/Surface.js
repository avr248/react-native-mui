...
const MD2Surface = forwardRef(({ style, theme: overrideTheme, ...rest }, ref) => {
    const { elevation = 4 } = (StyleSheet.flatten(style) || {});
    const { dark: isDarkTheme, mode, colors } = useInternalTheme(overrideTheme);
    return (React.createElement(Animated.View, { ref: ref, ...rest, style: [
            {
...
const outerLayerStyleProperties = [
    'position',
    'alignSelf',
    'top',
    'right',
...
const SurfaceIOS = forwardRef(({ elevation, style, backgroundColor, testID, children, mode = 'elevated', ...props }, ref) => {
    const [outerLayerViewStyles, innerLayerViewStyles] = React.useMemo(() => {
        const flattenedStyles = (StyleSheet.flatten(style) || {});
        const [filteredStyles, outerLayerStyles, borderRadiusStyles] = splitStyles(flattenedStyles, (style) => outerLayerStyleProperties.includes(style) ||
            style.startsWith('margin'), (style) => style.startsWith('border') && style.endsWith('Radius'));
    ...
    return (React.createElement(Animated.View, { ref: ref, style: outerLayerViewStyles, testID: `${testID}-outer-layer` },
        React.createElement(Animated.View, { ...props, style: innerLayerViewStyles, testID: testID }, children)));
});
/**
 ...
 * ```
 */
const Surface = forwardRef(({ elevation = 1, children, theme: overridenTheme, style, testID = 'surface', mode = 'elevated', ...props }, ref) => {
    const theme = useInternalTheme(overridenTheme);
    if (!theme.isV3)
        return (React.createElement(MD2Surface, { ...props, theme: theme, style: style, ref: ref }, children));
    const { colors } = theme;
    const inputRange = [0, 1, 2, 3, 4, 5];
    ...
    if (Platform.OS === 'web') {
        ...
        return (React.createElement(Animated.View, { ...props, pointerEvents: pointerEvents, ref: ref, testID: testID, style: [
                { backgroundColor },
    ...
    if (Platform.OS === 'android') {
        ...
        return (React.createElement(Animated.View, { ...props, testID: testID, ref: ref, style: [
                {
                    backgroundColor,
                    transform,
                },
    ...
    return (React.createElement(SurfaceIOS, { ...props, ref: ref, elevation: elevation, backgroundColor: backgroundColor, style: style, testID: testID, mode: mode }, children));
});
