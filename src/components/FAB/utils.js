...
export const getCombinedStyles = ({ isAnimatedFromRight, isIconStatic, distance, animFAB, }) => {
...
const getBackgroundColor = ({ theme, isVariant, disabled, customBackgroundColor, }) => {
...
const getForegroundColor = ({ theme, isVariant, disabled, backgroundColor, customColor, }) => {
...
export const getFABColors = ({ theme, variant, disabled, customColor, customBackgroundColor, customRippleColor, }) => {
...
const getBackdropColor = ({ theme, customBackdropColor, }) => {
...
const getStackedFABBackgroundColor = ({ theme }) => {
    if (theme.isV3) {
        return theme.colors.elevation.level3;
    }
    return theme.colors.surface;
};
export const getFABGroupColors = ({ theme, customBackdropColor, }) => {
    return {
...
export const getFabStyle = ({ size, theme, customSize, }) => {
    ...
    if (customSize)
        return getCustomFabSize(customSize, roundness);
    if (isV3) {
        switch (size) {
            case 'small':
...
export const getExtendedFabStyle = ({ customSize, theme, }) => {
    if (customSize)
        return getExtendedFabDimensions(customSize);
    const { isV3 } = theme;
