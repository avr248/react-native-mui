...
import { getFlatInputColors, getOutlinedInputColors, } from '../TextInput/helpers';
...
it('correctly renders left-side icon adornment, and right-side affix adornment', () => {
    const { getByText, getByTestId, toJSON } = render(React.createElement(TextInput, { label: "Flat input", placeholder: "Type something", value: 'Some test value', left: React.createElement(TextInput.Icon, { icon: "heart", onPress: () => {
                console.log('!@# press left');
            } }), right: React.createElement(TextInput.Affix, { text: affixTextValue, textStyle: style.inputStyle }) }));
...
it('correctly renders left-side affix adornment, and right-side icon adornment', () => {
    const { getByText, getByTestId, toJSON } = render(React.createElement(TextInput, { label: "Flat input", placeholder: "Type something", value: 'Some test value', left: React.createElement(TextInput.Affix, { text: affixTextValue, textStyle: style.inputStyle }), right: React.createElement(TextInput.Icon, { icon: "heart", onPress: () => {
                console.log('!@# press left');
            } }) }));
    expect(getByText(affixTextValue)).toBeTruthy();
...
it('correctly applies default textAlign based on default RTL', () => {
    const { toJSON } = render(React.createElement(TextInput, { label: "Flat input", placeholder: "Type something", value: 'Some test value' }));
    expect(toJSON()).toMatchSnapshot();
});
it('correctly applies textAlign center', () => {
    const { toJSON } = render(React.createElement(TextInput, { label: "Flat input", placeholder: "Type something", value: 'Some test value', style: style.centered }));
    expect(toJSON()).toMatchSnapshot();
});
it('correctly applies cursorColor prop', () => {
    const { toJSON } = render(React.createElement(TextInput, { label: "Flat input", placeholder: "Type something", value: 'Some test value', cursorColor: red500 }));
    expect(toJSON()).toMatchSnapshot();
});
it('correctly applies height to multiline Outline TextInput', () => {
    const { toJSON } = render(React.createElement(TextInput, { mode: "outlined", label: "Outline Input", placeholder: "Type Something", value: 'Some test value', style: style.height, multiline: true }));
    expect(toJSON()).toMatchSnapshot();
});
it('correctly applies error state Outline TextInput', () => {
    const { getByTestId } = render(React.createElement(TextInput, { mode: "outlined", label: "Outline Input with error", placeholder: "Type Something", value: 'Some test value', error: true }));
...
it('contains patch spacing for flat input when ios, multiline and disabled', () => {
    Platform.OS = 'ios';
    const { getByTestId } = render(React.createElement(TextInput, { label: "Flat input", multiline: true, placeholder: "Type something", value: 'Some test value' }));
    expect(getByTestId('patch-container')).toBeTruthy();
});
it('correctly applies a component as the text label', () => {
    const { toJSON } = render(React.createElement(TextInput, { label: React.createElement(Text, { style: style.inputStyle }, "Flat input"), placeholder: "Type something", value: 'Some test value' }));
    expect(toJSON()).toMatchSnapshot();
});
it('correctly applies paddingLeft from contentStyleProp', () => {
    const { toJSON } = render(React.createElement(TextInput, { label: "With padding", placeholder: "Type something", value: 'Some test value', contentStyle: style.contentStyle }));
    expect(toJSON()).toMatchSnapshot();
});
it('renders label with correct color when active', () => {
    const { getByTestId } = render(React.createElement(TextInput, { label: "Flat input", placeholder: "Type something", value: 'Some test value', testID: 'text-input-flat' }));
...
it('renders label with correct color when inactive', () => {
    const { getByTestId } = render(React.createElement(TextInput, { label: "Flat input", placeholder: "Type something", value: 'Some test value', testID: 'text-input' }));
...
it('renders input placeholder initially with an empty space character', () => {
    const { getByTestId } = render(React.createElement(TextInput, { multiline: true, label: "Multiline input", testID: 'text-input' }));
...
it('correctly applies padding offset to input label on Android when RTL', () => {
    ...
    const { getByTestId } = render(React.createElement(TextInput, { label: "Flat input", mode: "flat", testID: "text-input-flat", left: React.createElement(TextInput.Affix, { text: affixTextValue, textStyle: style.inputStyle }), right: React.createElement(TextInput.Affix, { text: affixTextValue, textStyle: style.inputStyle }) }));
    expect(getByTestId('text-input-flat-label-active')).toHaveStyle({
        paddingLeft: 56,
        paddingRight: 16,
    });
...
it('calls onLayout on right-side affix adornment', () => {
    ...
    const { getByTestId } = render(React.createElement(TextInput, { label: "Flat input", placeholder: "Type something", value: 'Some test value', right: React.createElement(TextInput.Affix, { text: affixTextValue, onLayout: onLayoutMock }) }));
    fireEvent(getByTestId('right-affix-adornment-text'), 'onLayout', nativeEventMock);
    expect(onLayoutMock).toHaveBeenCalledWith(nativeEventMock);
});
['outlined', 'flat'].forEach((mode) => it(`renders ${mode} input with correct line height`, () => {
    const input = render(React.createElement(TextInput, { mode: mode, multiline: true, label: "Flat input", testID: `text-input-${mode}`, style: style.lineHeight }));
    expect(input.getByTestId(`text-input-${mode}`)).toHaveStyle({
        lineHeight: 22,
    });
}));
['outlined', 'flat'].forEach((mode) => it(`renders ${mode} input with passed textColor`, () => {
    const input = render(React.createElement(TextInput, { mode: mode, multiline: true, label: "Flat input", testID: `text-input-${mode}`, style: style.lineHeight, textColor: 'purple' }));
    expect(input.getByTestId(`text-input-${mode}`)).toHaveStyle({
        color: 'purple',
    });
}));
it("correctly applies theme background to label when input's background is transparent", () => {
    ...
    const { getByTestId } = render(React.createElement(ThemeProvider, { theme: theme },
        React.createElement(TextInput, { mode: "outlined", label: "Transparent input", value: 'Some test value', style: { backgroundColor }, testID: 'transparent-example' })));
...
describe('maxFontSizeMultiplier', () => {
    const createInput = (type, maxFontSizeMultiplier) => {
        return (React.createElement(TextInput, { mode: type, maxFontSizeMultiplier: maxFontSizeMultiplier }));
    };
    it('should have default value in flat input', () => {
        const { getByTestId } = render(createInput('flat'));
        expect(getByTestId('text-input-flat').props.maxFontSizeMultiplier).toBe(1.5);
    });
    it('should have default value in outlined input', () => {
        const { getByTestId } = render(createInput('outlined'));
        expect(getByTestId('text-input-outlined').props.maxFontSizeMultiplier).toBe(1.5);
    });
    ...
    it('should have correct passed value in outlined input', () => {
        ...
        expect(getByTestId('text-input-outlined').props.maxFontSizeMultiplier).toBe(2);
    });
    it('should have passed null value in flat input', () => {
        const { getByTestId } = render(createInput('flat', null));
        expect(getByTestId('text-input-flat').props.maxFontSizeMultiplier).toBe(null);
    });
    it('should have passed null value in outlined input', () => {
        const { getByTestId } = render(createInput('outlined', null));
        expect(getByTestId('text-input-outlined').props.maxFontSizeMultiplier).toBe(null);
    });
});
describe('getFlatInputColor - underline color', () => {
    it('should return correct disabled color, for theme version 3', () => {
        expect(getFlatInputColors({
            disabled: true,
            theme: getTheme(),
        })).toMatchObject({
...
describe('outlineStyle - underlineStyle', () => {
    it('correctly applies outline style', () => {
        const { getByTestId } = render(React.createElement(TextInput, { mode: "outlined", outlineStyle: { borderRadius: 16, borderWidth: 6 } }));
    ...
    it('correctly applies underline style', () => {
        const { getByTestId } = render(React.createElement(TextInput, { mode: "flat", underlineStyle: { borderRadius: 16, borderWidth: 6 } }));
