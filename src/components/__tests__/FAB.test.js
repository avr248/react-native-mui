...
it('renders default FAB', () => {
    const tree = render(React.createElement(FAB, { onPress: () => { }, icon: "plus" })).toJSON();
    expect(tree).toMatchSnapshot();
});
it('renders small FAB', () => {
    const tree = render(React.createElement(FAB, { size: "small", onPress: () => { }, icon: "plus" })).toJSON();
    expect(tree).toMatchSnapshot();
});
it('renders large FAB', () => {
    const tree = render(React.createElement(FAB, { size: "large", onPress: () => { }, icon: "plus" })).toJSON();
    expect(tree).toMatchSnapshot();
});
it('renders FAB with custom size prop', () => {
    const tree = render(React.createElement(FAB, { customSize: 100, onPress: () => { }, icon: "plus" })).toJSON();
    expect(tree).toMatchSnapshot();
});
it('renders extended FAB', () => {
    const tree = render(React.createElement(FAB, { onPress: () => { }, icon: "plus", label: "Add items" })).toJSON();
    expect(tree).toMatchSnapshot();
});
it('renders extended FAB with custom size prop', () => {
    const tree = render(React.createElement(FAB, { customSize: 100, onPress: () => { }, icon: "plus", label: "Add items" })).toJSON();
    expect(tree).toMatchSnapshot();
});
it('renders loading FAB', () => {
    const tree = render(React.createElement(FAB, { onPress: () => { }, icon: "plus", loading: true })).toJSON();
    expect(tree).toMatchSnapshot();
});
it('renders loading FAB with custom size prop', () => {
    const tree = render(React.createElement(FAB, { customSize: 100, onPress: () => { }, icon: "plus", loading: true })).toJSON();
    expect(tree).toMatchSnapshot();
});
it('renders disabled FAB', () => {
    const tree = render(React.createElement(FAB, { onPress: () => { }, icon: "plus", disabled: true })).toJSON();
    expect(tree).toMatchSnapshot();
});
it('renders custom color for the icon and label of the FAB', () => {
    const tree = render(React.createElement(FAB, { onPress: () => { }, icon: "plus", color: "#AA0114" })).toJSON();
    expect(tree).toMatchSnapshot();
});
it('renders not visible FAB', () => {
    const { update, toJSON } = render(React.createElement(FAB, { onPress: () => { }, icon: "plus" }));
    update(React.createElement(FAB, { onPress: () => { }, icon: "plus", visible: false }));
    expect(toJSON()).toMatchSnapshot();
});
it('renders visible FAB', () => {
    const { update, toJSON } = render(React.createElement(FAB, { onPress: () => { }, icon: "plus", visible: false }));
    update(React.createElement(FAB, { onPress: () => { }, icon: "plus", visible: true }));
    expect(toJSON()).toMatchSnapshot();
});
it('renders FAB with custom border radius', () => {
    const { getByTestId } = render(React.createElement(FAB, { onPress: () => { }, icon: "plus", testID: "fab", style: styles.borderRadius }));
    expect(getByTestId('fab-container')).toHaveStyle({ borderRadius: 0 });
});
it('renders FAB without uppercase styling by default', () => {
    const { getByTestId } = render(React.createElement(FAB, { onPress: () => { }, label: "Add items", testID: "fab" }));
    expect(getByTestId('fab-text')).not.toHaveStyle({
        textTransform: 'uppercase',
    });
});
it('renders FAB without uppercase styling if uppercase prop is falsy', () => {
    const { getByTestId } = render(React.createElement(FAB, { onPress: () => { }, label: "Add items", testID: "fab", uppercase: false }));
    expect(getByTestId('fab-text')).not.toHaveStyle({
        textTransform: 'uppercase',
    });
});
it('renders FAB with uppercase styling if uppercase prop is truthy', () => {
    const { getByTestId } = render(React.createElement(FAB, { onPress: () => { }, label: "Add items", testID: "fab", uppercase: true }));
...
['small', 'medium', 'large'].forEach((size) => {
    it(`renders ${size} FAB with correct size and border radius`, () => {
        const { getByTestId } = render(React.createElement(FAB, { onPress: () => { }, size: size, icon: "plus", testID: `${size}-fab` }));
...
describe('getFABColors - foreground color', () => {
    ...
    it('should return correct theme color, for theme version 2', () => {
        expect(getFABColors({
            ...
            foregroundColor: getContrastingColor(getTheme(false, false).colors.accent, white, 'rgba(0, 0, 0, .54)'),
        });
    });
});
it('animated value changes correctly', () => {
    const value = new Animated.Value(1);
    const { getByTestId } = render(React.createElement(FAB, { size: "small", onPress: () => { }, icon: "plus", testID: "fab", style: [{ transform: [{ scale: value }] }] }));
...
describe('FAB events', () => {
    it('onPress passes event', () => {
        ...
        const { getByText } = render(React.createElement(FAB, { onPress: onPress, label: "Add items" }));
        act(() => {
    ...
    it('onLongPress passes event', () => {
        ...
        const { getByText } = render(React.createElement(FAB, { onLongPress: onLongPress, label: "Add items" }));
        act(() => {
