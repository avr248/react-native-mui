...
it('renders avatar with text', () => {
    const tree = render(React.createElement(Avatar.Text, { label: "XD" })).toJSON();
    expect(tree).toMatchSnapshot();
});
it('renders avatar with text and custom size', () => {
    const tree = render(React.createElement(Avatar.Text, { size: 96, label: "XD" })).toJSON();
    expect(tree).toMatchSnapshot();
});
it('renders avatar with text and custom background color', () => {
    const tree = render(React.createElement(Avatar.Text, { style: styles.bgColor, label: "XD" })).toJSON();
    expect(tree).toMatchSnapshot();
});
it('renders avatar with text and custom colors', () => {
    const tree = render(React.createElement(Avatar.Text, { color: "#FFFFFF", label: "XD" })).toJSON();
    expect(tree).toMatchSnapshot();
});
it('renders avatar with icon', () => {
    const tree = render(React.createElement(Avatar.Icon, { icon: "information" })).toJSON();
    expect(tree).toMatchSnapshot();
});
it('renders avatar with icon and custom background color', () => {
    const tree = render(React.createElement(Avatar.Icon, { style: styles.bgColor, icon: "information" })).toJSON();
    expect(tree).toMatchSnapshot();
});
it('renders avatar with image', () => {
    const tree = render(React.createElement(Avatar.Image, { source: { uri: 'avatar.png' } })).toJSON();
    expect(tree).toMatchSnapshot();
});
describe('AvatarImage listener', () => {
    const onListenerMock = jest.fn();
    it('onError should be called', () => {
        const { getByTestId } = render(React.createElement(Avatar.Image, { testID: 'avatar-image', source: { uri: 'avatar.png' }, onError: onListenerMock, onLayout: onListenerMock, onLoad: onListenerMock, onLoadEnd: onListenerMock, onLoadStart: onListenerMock, onProgress: onListenerMock }));
