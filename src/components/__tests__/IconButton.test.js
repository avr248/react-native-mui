...
it('renders icon button by default', () => {
    const tree = render(React.createElement(IconButton, { icon: "camera" })).toJSON();
    expect(tree).toMatchSnapshot();
});
it('renders icon button with color', () => {
    const tree = render(React.createElement(IconButton, { icon: "camera", iconColor: pink500 })).toJSON();
    expect(tree).toMatchSnapshot();
});
it('renders icon button with size', () => {
    const tree = render(React.createElement(IconButton, { icon: "camera", size: 30 })).toJSON();
    expect(tree).toMatchSnapshot();
});
it('renders disabled icon button', () => {
    const tree = render(React.createElement(IconButton, { icon: "camera", disabled: true })).toJSON();
    expect(tree).toMatchSnapshot();
});
it('renders icon change animated', () => {
    const tree = render(React.createElement(IconButton, { icon: "camera", animated: true })).toJSON();
    expect(tree).toMatchSnapshot();
});
it('renders icon button with custom border radius', () => {
    const { getByTestId } = render(React.createElement(IconButton, { icon: "camera", testID: "icon-button", size: 36, onPress: () => { }, style: styles.square }));
...
describe('getIconButtonColor - background color', () => {
    ...
    ['contained', 'contained-tonal'].forEach((mode) => it(`should return correct disabled color, for theme version 3, ${mode} mode`, () => {
        expect(getIconButtonColor({
            theme: getTheme(),
            mode,
            disabled: true,
...
it('action animated value changes correctly', () => {
    ...
    const { getByTestId } = render(React.createElement(IconButton, { icon: "menu", style: [{ transform: [{ scale: value }] }], testID: "icon-button" }));
