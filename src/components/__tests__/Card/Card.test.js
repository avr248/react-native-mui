...
describe('Card', () => {
    it('renders an outlined card', () => {
        const tree = render(React.createElement(Card, { mode: "outlined" }, null)).toJSON();
        expect(tree).toMatchSnapshot();
    });
    it('renders an outlined card with custom border radius and color', () => {
        const { getByTestId } = render(React.createElement(Card, { mode: "outlined", theme: { colors: { outline: 'purple' } }, style: styles.customBorderRadius }, null));
    ...
    it('renders an outlined card with custom border color', () => {
        const { getByLabelText } = render(React.createElement(Card, { mode: "outlined", accessibilityLabel: "card", style: { borderColor: MD3Colors.error50 } }, null));
        expect(getByLabelText('card')).toHaveStyle({
            borderColor: MD3Colors.error50,
        });
    });
    it('renders with a custom theme', () => {
        const { getByLabelText } = render(React.createElement(Card, { mode: "outlined", accessibilityLabel: "card", theme: { colors: { surface: '#0000FF' } } }, null));
        expect(getByLabelText('card')).toHaveStyle({
            backgroundColor: '#0000FF',
        });
    });
    it('renders with a content style', () => {
        const { getByTestId } = render(React.createElement(Card, { contentStyle: styles.contentStyle },
            React.createElement(Text, null, "Content")));
        expect(getByTestId('card')).toHaveStyle(styles.contentStyle);
    });
    it('does not render a disabled accessibility state', () => {
        const { getByTestId } = render(React.createElement(Card, null, null));
        expect(getByTestId('card').props.accessibilityState || {}).not.toMatchObject({
            disabled: true,
        });
    });
    it('does render a disabled accessibility state', () => {
        const { getByTestId } = render(React.createElement(Card, { onPress: () => { }, disabled: true }, null));
...
describe('CardCover', () => {
    it('renders with custom border radius', () => {
        const { getByTestId } = render(React.createElement(Card, null,
            React.createElement(Card.Cover, { testID: "card-cover", style: styles.customCoverRadius })));
...
describe('CardActions', () => {
    it('renders button with passed mode', () => {
        const { getByTestId } = render(React.createElement(Card, null,
            React.createElement(Card.Actions, { testID: "card-actions" },
                React.createElement(Button, { mode: "contained" }, "Agree"))));
        expect(getByTestId('card-actions').props.children[0].props.mode).toBe('contained');
    });
    it('renders button with custom styles', () => {
        const { getByTestId } = render(React.createElement(Card, null,
            React.createElement(Card.Actions, null,
                React.createElement(Button, { testID: "card-actions-button", mode: "contained", style: styles.customBorderRadius }, "Agree"))));
...
it('animated value changes correctly', () => {
    ...
    const { getByTestId } = render(React.createElement(Card, { mode: "outlined", accessibilityLabel: "card", style: [{ transform: [{ scale: value }] }] }, null));
