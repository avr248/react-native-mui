...
import { getDisabledSegmentedButtonStyle, getSegmentedButtonColors, } from '../SegmentedButtons/utils';
it('renders segmented button', () => {
    const tree = render(React.createElement(SegmentedButtons, { onValueChange: () => { }, value: 'walk', buttons: [{ value: 'walk' }, { value: 'ride' }] })).toJSON();
    expect(tree).toMatchSnapshot();
});
it('renders disabled segmented button', async () => {
    const tree = render(React.createElement(SegmentedButtons, { onValueChange: () => { }, value: 'walk', buttons: [{ value: 'walk' }, { value: 'ride', disabled: true }] })).toJSON();
    process.nextTick(() => {
        expect(tree).toMatchSnapshot();
    });
});
it('renders checked segmented button with selected check', async () => {
    const tree = render(React.createElement(SegmentedButtons, { onValueChange: () => { }, value: 'walk', buttons: [
...
describe('getSegmentedButtonColors', () => {
  ...
  `('returns $expected when disabled: $disabled, checked: $checked, checkedColor is $checkedColor and uncheckedColor is $uncheckedColor  and isV3: $theme.isV3', ({ theme, disabled, checked, checkedColor, uncheckedColor, expected }) => {
        expect(getSegmentedButtonColors({
            theme,
            disabled,
            checked,
            checkedColor,
...
describe('getDisabledSegmentedButtonBorderWidth', () => {
    it('Returns empty style object for all enabled buttons', () => {
        [0, 1, 2].forEach((index) => {
            expect(getDisabledSegmentedButtonStyle({
                theme: getTheme(),
                buttons: [
                    { disabled: false },
                    { disabled: false },
...
describe('should render icon when', () => {
    it('icon prop is passed', () => {
        const { getByTestId } = render(React.createElement(SegmentedButtons, { value: 'walk', buttons: [
                {
                    icon: 'walk',
                    value: 'walk',
...
describe('should not render icon when', () => {
    it('icon prop is not passed', () => {
        const { queryByTestId } = render(React.createElement(SegmentedButtons, { value: 'walk', buttons: [
                {
                    value: 'walk',
                    testID: 'walking-button',
                },
    ...
    it('icon prop is passed along with label, button is checked, showSelectedCheck is true', () => {
        const { getByTestId, queryByTestId } = render(React.createElement(SegmentedButtons, { value: 'walk', buttons: [
                {
                    icon: 'walk',
                    label: 'Walking',
                    value: 'walk',
...
describe('should have `accessibilityState={ checked: true }` when selected', () => {
    it('should have two button selected', () => {
        ...
        const { getAllByA11yState } = render(React.createElement(SegmentedButtons, { multiSelect: true, value: ['walk', 'transit'], buttons: [
    ...
    it('show selected check icon should be shown', () => {
        const onValueChange = jest.fn();
        const { getByTestId } = render(React.createElement(SegmentedButtons, { multiSelect: true, value: ['walk', 'transit'], buttons: [
                {
                    value: 'walk',
                    label: 'Walking',
                    showSelectedCheck: true,
