...
const DummyComponent = React.forwardRef((props, ref) => (React.createElement(View, { ...props, ref: ref },
    React.createElement(Text, null, "dummy component"))));
describe('Tooltip', () => {
    const setup = (propOverrides, measure = {}) => {
        const defaultProps = {
            children: React.createElement(DummyComponent, null),
        ...
        const wrapper = render(React.createElement(PaperProvider, null,
            React.createElement(Tooltip, { ...defaultProps })));
        return { wrapper };
    };
    describe('Mobile', () => {
        beforeAll(() => {
        ...
        describe('pressOut', () => {
            ...
            it('hides the tooltip when the user stop pressing the component', async () => {
                const { wrapper: { queryByText, getByText, findByText }, } = setup({ enterTouchDelay: 50, leaveTouchDelay: 0 });
                fireEvent(getByText('dummy component'), 'longPress');
                await findByText('some tooltip text');
                fireEvent(getByText('dummy component'), 'pressOut');
        ...
        describe('Tooltip position', () => {
            ...
            describe('When it does not overflow', () => {
                it('centers the tooltip in the middle of the children component', async () => {
                    const { wrapper: { getByText, getByTestId, findByText }, } = setup();
                    fireEvent(getByText('dummy component'), 'longPress');
            ...
            describe('When it overflows to left', () => {
                it('renders the tooltip with the right placement', async () => {
                    const { wrapper: { getByText, getByTestId, findByText }, } = setup({}, { pageX: 0 }); // Component starting at the starting 0 X coord
                    fireEvent(getByText('dummy component'), 'longPress');
                    fireEvent(await findByText('some tooltip text'), 'layout', {
                        nativeEvent: {
                            layout: { width: TOOLTIP_WIDTH, height: TOOLTIP_HEIGHT },
            ...
            describe('When it overflows to right', () => {
                it('renders the tooltip with the right placement', async () => {
                    const { wrapper: { getByText, getByTestId, findByText }, } = setup({}, { pageX: 900, width: 150 }); // Component close to the screen limit
                    fireEvent(getByText('dummy component'), 'longPress');
                    fireEvent(await findByText('some tooltip text'), 'layout', {
                        nativeEvent: {
                            layout: { width: TOOLTIP_WIDTH, height: TOOLTIP_HEIGHT },
            ...
            describe('When it overflows to bottom', () => {
                it('renders the tooltip with the right placement', async () => {
                    const { wrapper: { getByText, getByTestId, findByText }, } = setup({}, { pageY: 600, height: 50 });
                    fireEvent(getByText('dummy component'), 'longPress');
                    fireEvent(await findByText('some tooltip text'), 'layout', {
                        nativeEvent: {
                            layout: { width: TOOLTIP_WIDTH, height: TOOLTIP_HEIGHT },
