...
class ClassProgressBar extends React.Component {
    render() {
        return React.createElement(ProgressBar, { ...this.props });
    }
}
...
it('renders progress bar with animated value', async () => {
    const tree = render(React.createElement(AnimatedProgressBar, { animatedValue: 0.2 }));
    await waitFor(() => tree.getByRole(a11yRole).props.onLayout(layoutEvent));
    tree.update(React.createElement(AnimatedProgressBar, { animatedValue: 0.4 }));
    expect(tree.container.props['animatedValue']).toBe(0.4);
});
it('renders progress bar with specific progress', async () => {
    const tree = render(React.createElement(ProgressBar, { progress: 0.2 }));
    await waitFor(() => tree.getByRole(a11yRole).props.onLayout(layoutEvent));
    expect(tree.toJSON()).toMatchSnapshot();
});
it('renders hidden progress bar', async () => {
    const tree = render(React.createElement(ProgressBar, { progress: 0.2, visible: false }));
    await waitFor(() => tree.getByRole(a11yRole).props.onLayout(layoutEvent));
    expect(tree.toJSON()).toMatchSnapshot();
});
it('renders colored progress bar', async () => {
    const tree = render(React.createElement(ProgressBar, { progress: 0.2, color: "red" }));
    await waitFor(() => tree.getByRole(a11yRole).props.onLayout(layoutEvent));
    expect(tree.toJSON()).toMatchSnapshot();
});
it('renders indeterminate progress bar', async () => {
    const tree = render(React.createElement(ProgressBar, { indeterminate: true }));
    await waitFor(() => tree.getByRole(a11yRole).props.onLayout(layoutEvent));
    expect(tree.toJSON()).toMatchSnapshot();
});
