...
import { white, black, grey400, grey50, grey800, pink500, grey700, } from '../../styles/themes/v2/colors';
import Switch from '../Switch/Switch';
import { getSwitchColor } from '../Switch/utils';
it('renders on switch', () => {
    const tree = render(React.createElement(Switch, { value: true })).toJSON();
    expect(tree).toMatchSnapshot();
});
it('renders off switch', () => {
    const tree = render(React.createElement(Switch, { value: false })).toJSON();
    expect(tree).toMatchSnapshot();
});
it('renders disabled switch', () => {
    const tree = render(React.createElement(Switch, { disabled: true, value: true })).toJSON();
    expect(tree).toMatchSnapshot();
});
it('renders switch with color', () => {
    const tree = render(React.createElement(Switch, { value: true, color: pink500 })).toJSON();
    expect(tree).toMatchSnapshot();
});
describe('getSwitchColor - checked color', () => {
    it('should return custom color', () => {
        expect(getSwitchColor({
            theme: getTheme(),
            color: 'purple',
        })).toMatchObject({
            checkedColor: 'purple',
