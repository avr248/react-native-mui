...
describe('Menu Item', () => {
    it('renders menu item', () => {
        const tree = render(React.createElement(React.Fragment, null,
            React.createElement(Menu.Item, { leadingIcon: "redo", onPress: () => { }, title: "Redo" }),
            React.createElement(Menu.Item, { leadingIcon: "undo", onPress: () => { }, title: "Undo" }),
            React.createElement(Menu.Item, { leadingIcon: "content-cut", onPress: () => { }, title: "Cut", disabled: true }),
            React.createElement(Menu.Item, { leadingIcon: "content-copy", onPress: () => { }, title: "Copy", disabled: true }),
            React.createElement(Menu.Item, { onPress: () => { }, title: "Paste" }))).toJSON();
        expect(tree).toMatchSnapshot();
    });
    it('should have titleMaxFontSizeMultiplier passed to title', () => {
        const labelMaxFontSizeMultiplier = 2;
        const { getByTestId } = render(React.createElement(Menu.Item, { titleMaxFontSizeMultiplier: labelMaxFontSizeMultiplier, leadingIcon: "content-cut", onPress: () => { }, title: "Cut" }));
        expect(getByTestId('menu-item-title').props.maxFontSizeMultiplier).toBe(labelMaxFontSizeMultiplier);
    });
    it('accepts different values for accessibilityState', () => {
        const { getByTestId } = render(React.createElement(Menu.Item, { accessibilityState: { checked: true }, title: "Option 1", testID: "touchable" }));
