...
describe('Surface', () => {
    it('should properly render passed props', () => {
        ...
        const { getByTestId } = render(React.createElement(Surface, { pointerEvents: "box-none", testID: testID }, null));
    ...
    describe('on iOS', () => {
        ...
        it('should render Surface with appropriate bg color but without shadow, if mode is set to "flat"', () => {
            const { getByTestId } = render(React.createElement(Surface, { mode: "flat", elevation: 5, pointerEvents: "box-none", testID: 'surface-test' }, null));
    ...
    `('applies $property to outer layer only', ({ property, value }) => {
            ...
            const { getByTestId } = render(React.createElement(Surface, { testID: "surface-test", style: style }, null));
    ...
    `('applies $property to every layer', ({ property, value }) => {
        ...
        describe('outer layer', () => {
            it('should not render rest style', () => {
                ...
                const { getByTestId } = render(React.createElement(Surface, { testID: testID, style: styles.restStyle }, null));
                expect(getByTestId(`${testID}-outer-layer`)).not.toHaveStyle(styles.restStyle);
            });
            it('should render absolute position properties on outer layer', () => {
                const testID = 'surface-test';
                const { getByTestId } = render(React.createElement(Surface, { testID: testID, style: styles.absoluteStyles }, null));
                expect(getByTestId(`${testID}-outer-layer`)).toHaveStyle(styles.absoluteStyles);
            });
        ...
        describe('inner layer', () => {
            it('should render inner layer styles on the inner layer', () => {
                const testID = 'surface-test';
                const { getByTestId } = render(React.createElement(Surface, { testID: testID, style: styles.innerLayerViewStyle }, null));
        ...
        it('applies backgroundColor to every layer', () => {
            ...
            const { getByTestId } = render(React.createElement(Surface, { testID: "surface-test", theme: { colors: { elevation: { level1: backgroundColor } } } }, null));
        ...
        describe('children wrapper', () => {
            it('should render rest styles', () => {
                ...
                const { getByTestId } = render(React.createElement(Surface, { testID: testID, style: combinedStyles }, null));
    ...
    describe('on Android', () => {
        it('should render Surface with appropriate bg color but without shadow, if mode is set to "flat"', () => {
            Platform.OS = 'android';
            const testID = 'surface-container';
            const { getByTestId } = render(React.createElement(Surface, { mode: "flat", elevation: 5, pointerEvents: "box-none", testID: testID }, null));
