...
const getAndroidCheckedColor = ({ theme, customColor, }) => {
    if (customColor) {
        return customColor;
    }
    if (theme.isV3) {
...
const getAndroidUncheckedColor = ({ theme, customUncheckedColor, }) => {
...
const getAndroidRippleColor = ({ theme, checkedColor, disabled, }) => {
    if (disabled) {
        if (theme.isV3) {
...
const getAndroidControlColor = ({ theme, checked, disabled, checkedColor, uncheckedColor, }) => {
    if (disabled) {
        if (theme.isV3) {
...
export const getAndroidSelectionControlColor = ({ theme, disabled, checked, customColor, customUncheckedColor, }) => {
...
const getIOSCheckedColor = ({ theme, disabled, customColor, }) => {
    if (disabled) {
        if (theme.isV3) {
            return theme.colors.onSurfaceDisabled;
        }
...
const getIOSRippleColor = ({ theme, checkedColor, disabled, }) => {
    if (disabled) {
        if (theme.isV3) {
            return color(theme.colors.onSurface).alpha(0.16).rgb().string();
        }
...
export const getSelectionControlIOSColor = ({ theme, disabled, customColor, }) => {
