...
export const getSegmentedButtonDensityPadding = ({ density, }) => {
...
export const getDisabledSegmentedButtonStyle = ({ theme, index, buttons, }) => {
...
export const getSegmentedButtonBorderRadius = ({ segment, theme, }) => {
    if (segment === 'first') {
        return {
...
const getSegmentedButtonBackgroundColor = ({ checked, theme }) => {
    if (checked) {
        if (theme.isV3) {
...
const getSegmentedButtonBorderColor = ({ theme, disabled, checked, }) => {
    if (theme.isV3) {
        if (disabled) {
...
const getSegmentedButtonBorderWidth = ({ theme, }) => {
    if (theme.isV3) {
        return 1;
    }
    return StyleSheet.hairlineWidth;
};
const getSegmentedButtonTextColor = ({ theme, disabled, checked, checkedColor, uncheckedColor, }) => {
    if (theme.isV3) {
        if (disabled) {
...
export const getSegmentedButtonColors = ({ theme, disabled, checked, checkedColor, uncheckedColor, }) => {
