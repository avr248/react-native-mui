...
const getCheckedColor = ({ theme, color, }) => {
    if (color) {
        return color;
    }
    if (theme.isV3) {
...
const getThumbTintColor = ({ theme, disabled, value, checkedColor, }) => {
...
const getOnTintColor = ({ theme, disabled, value, checkedColor, }) => {
    const isIOS = Platform.OS === 'ios';
    if (isIOS) {
        return checkedColor;
    }
...
export const getSwitchColor = ({ theme, disabled, value, color, }) => {
