 ...
 */
const TextInput = forwardRef(({ mode = 'flat', dense = false, disabled = false, error: errorProp = false, multiline = false, editable = true, contentStyle, render = DefaultRenderer, theme: themeOverrides, ...rest }, ref) => {
    const theme = useInternalTheme(themeOverrides);
    const isControlled = rest.value !== undefined;
    const validInputValue = isControlled ? rest.value : rest.defaultValue;
    const { current: labeled } = React.useRef(new Animated.Value(validInputValue ? 0 : 1));
    const { current: error } = React.useRef(new Animated.Value(errorProp ? 1 : 0));
    ...
    if (mode === 'outlined') {
        return (React.createElement(TextInputOutlined, { dense: dense, disabled: disabled, error: errorProp, multiline: multiline, editable: editable, render: render, ...rest, theme: theme, value: value, parentState: {
                labeled,
                error,
                focused,
                placeholder,
                ...
                root.current = ref;
            }, onFocus: handleFocus, forceFocus: forceFocus, onBlur: handleBlur, onChangeText: handleChangeText, onLayoutAnimatedText: handleLayoutAnimatedText, onLeftAffixLayoutChange: onLeftAffixLayoutChange, onRightAffixLayoutChange: onRightAffixLayoutChange, maxFontSizeMultiplier: maxFontSizeMultiplier, contentStyle: contentStyle }));
    }
    return (React.createElement(TextInputFlat, { dense: dense, disabled: disabled, error: errorProp, multiline: multiline, editable: editable, render: render, ...rest, theme: theme, value: value, parentState: {
            labeled,
            error,
            focused,
            placeholder,
