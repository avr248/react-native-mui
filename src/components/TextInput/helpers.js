...
export const adjustPaddingOut = ({ pad, multiline, label, scale, height, fontSize, lineHeight, dense, offset, isAndroid, }) => {
...
export function calculateFlatAffixTopPosition({ height, paddingTop, paddingBottom, affixHeight, }) {
    const inputHeightWithoutPadding = height - paddingTop - paddingBottom;
    const halfOfTheInputHeightDecreasedByAffixHeight = (inputHeightWithoutPadding - affixHeight) / 2;
    return paddingTop + halfOfTheInputHeightDecreasedByAffixHeight;
}
export function calculateOutlinedIconAndAffixTopPosition({ height, affixHeight, labelYOffset, }) {
    return (height - affixHeight + labelYOffset) / 2;
}
export const calculateFlatInputHorizontalPadding = ({ adornmentConfig, isV3, }) => {
    const { LABEL_PADDING_HORIZONTAL, ADORNMENT_OFFSET, FLAT_INPUT_OFFSET } = getConstants(isV3);
...
const getInputTextColor = ({ theme, textColor, disabled, mode, }) => {
...
const getActiveColor = ({ theme, disabled, error, activeUnderlineColor, activeOutlineColor, mode, }) => {
    const isFlat = mode === 'flat';
...
const getPlaceholderColor = ({ theme, disabled }) => {
    if (theme.isV3) {
        if (disabled) {
            return theme.colors.onSurfaceDisabled;
        }
...
const getSelectionColor = ({ activeColor, customSelectionColor, }) => {
...
const getFlatBackgroundColor = ({ theme, disabled }) => {
    if (theme.isV3) {
        if (disabled) {
...
const getFlatUnderlineColor = ({ theme, disabled, underlineColor, }) => {
...
const getOutlinedOutlineInputColor = ({ theme, disabled, customOutlineColor, }) => {
...
export const getFlatInputColors = ({ underlineColor, activeUnderlineColor, customSelectionColor, textColor, disabled, error, theme, }) => {
...
export const getOutlinedInputColors = ({ activeOutlineColor, customOutlineColor, customSelectionColor, textColor, disabled, error, theme, }) => {
