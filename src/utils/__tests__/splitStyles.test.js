...
describe('splitStyles', () => {
    ...
    it('splits margins, paddings, and border radiuses correctly', () => {
        const marginPredicate = (style) => style.startsWith('margin');
        const paddingPredicate = (style) => style.startsWith('padding');
        const borderRadiusPredicate = (style) => style.startsWith('border') && style.endsWith('Radius');
        const [filteredStyles, marginStyles, paddingStyles, borderRadiusStyles] = splitStyles(styles, marginPredicate, paddingPredicate, borderRadiusPredicate);
    ...
    it('filtered styles is an empty object if all styles matched some predicate', () => {
        ...
        const [filteredStyles] = splitStyles(styles, (style) => style.startsWith('margin'), (style) => style.startsWith('padding'));
        expect(keysLength(filteredStyles)).toBe(0);
    });
    it('processes predicates in order', () => {
        const [, marginStyles, marginStyles2, marginStyles3] = splitStyles(styles, (style) => style.startsWith('margin'), (style) => style.startsWith('margin'), (style) => style.startsWith('margin'));
        expect(keysLength(marginStyles)).toBeGreaterThan(0);
